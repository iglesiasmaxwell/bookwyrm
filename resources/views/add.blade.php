...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            .status-btn:hover, .status-btn:focus, .status-btn:active, .btn-check:checked+.btn, .btn.active, .btn.show, .btn:first-child:active, :not(.btn-check)+.btn:active{
                border-width: 1.5px!important;
                color: #334155!important;
                border-color: #94a3b8b7!important;
...
<body class="antialiased bg-slate-200">
    ...
    <section>
        <nav class="navbar fixed-top bg-slate-500/80 ps-4 pe-2 py-2 d-lg-none d-md-block rounded-bottom-2">
        ...
        <div class="row g-0">
            <div class="col-lg-4 col-xl-3 bg-gradient-to-b from-slate-300 h-full d-none d-lg-block">
                <div class="py-4 pl-10">
                    <div class="fixed-top bg-slate-300/35 pt-3 pb-2.5 pl-10 col-3 rounded-bottom-4">
            ...
            <div class="col-sm-12 col-lg-8 col-xl-9 h-screen">
                <nav class="navbar">
                    <div class="container-fluid">
                        <p class="font-body font-semibold text-2xl mx-3 mt-20 lg:mt-5 text-slate-700 hover:text-slate-700 navbar-brand">Add book</p>
                    </div>
                </nav>
                <div class="mt-4 pb-2">
                    <div class="card mx-4 rounded-3 border-slate-400/75">
                        <div class="card-body bg-slate-300 font-body text-slate-700 rounded-3 shadow-md">
                            <form method="POST" action="{{ route('store.book') }}">
                                @csrf
                                <div class="form-group">
                                    <label class="text-lg font-medium" for="titleInput">Title <span class="text-xs">(required)</span></label>
                                    <input class="w-full bg-slate-200 outline-[0.5px] outline-slate-400/60 px-2 h-8 text-base rounded-3 mb-2" type="text" id="titleInput" name="title" autocomplete="off">
                                    <label class="text-lg font-medium" for="subtitleInput">Subtitle</label>
                                    <input class="w-full bg-slate-200 outline-[0.5px] outline-slate-400/60 px-2 h-8 text-base rounded-3 mb-2" type="text" id="subtitleInput" name="subtitle" autocomplete="off">
                                    <label class="text-lg font-medium" for="authorInput">Author <span class="text-xs">(required)</span></label>
                                    <input class="w-full bg-slate-200 outline-[0.5px] outline-slate-400/60 px-2 h-8 text-base rounded-3 mb-2" type="text" id="authorInput" name="author" autocomplete="off">
                                    <label class="text-lg font-medium" for="pageInput">Page</label>
                                    <input class="w-full bg-slate-200 outline-[0.5px] outline-slate-400/60 px-2 h-8 text-base rounded-3 mb-2" type="number" id="pageInput" name="pages" autocomplete="off" min="1">
                                    <label class="text-lg font-medium" for="yearInput">Year</label>
                                    <input class="w-full bg-slate-200 outline-[0.5px] outline-slate-400/60 px-2 h-8 text-base rounded-3 mb-2" type="number" id="yearInput" name="year" autocomplete="off" min="1">
                                    <label class="text-lg font-medium hidden" for="ratingInput" id="ratingLabel">Rating <span class="text-xs">(1-5)</span></label>
                                    <input class="w-full bg-slate-200 outline-[0.5px] outline-slate-400/60 px-2 h-8 text-base rounded-3 mb-2 hidden" type="number" id="ratingInput" name="rating" autocomplete="off" min="1" max="5">
                                    ...
                                    <div class="row g-0 text-center mb-3">
                                        ...
                                        <label class="text-lg font-slate-700 font-medium w-full sm:w-1/3 bg-slate-200 outline-[0.5px] outline-slate-400/60 px-2 py-1.5 text-base rounded-start-3 rounded-end-0 mb-2 btn status-btn" for="reading" onclick="unCheck('ratingInput', 'ratingLabel')">Reading</label>
                                        <input class="btn-check" type="radio" id="planning" value="planning" name="condition" autocomplete="off">
                                        <label class="text-lg font-slate-700 font-medium w-full sm:w-1/3 bg-slate-200 outline-[0.5px] outline-slate-400/60 px-2 py-1.5 text-base rounded-0 mb-2 btn status-btn" for="planning" onclick="unCheck('ratingInput', 'ratingLabel')">Planning</label>
                                        <input class="btn-check" type="radio" id="completed" value="completed" name="condition" autocomplete="off">
                                        <label class="text-lg font-slate-700 font-medium w-full sm:w-1/3 bg-slate-200 outline-[0.5px] outline-slate-400/60 px-2 py-1.5 text-base rounded-end-3 rounded-start-0 mb-2 btn status-btn" for="completed" onclick="Check('ratingInput', 'ratingLabel')">Completed</label>
                                    </div>
                                    <div class="row g-0 text-center pt-1">
                                        ...
                                        <button class="text-lg font-medium w-1/2 bg-slate-200 hover:bg-slate-100/75 outline-[0.5px] outline-slate-400/60 px-2 py-1.5 text-base rounded-end-3 mb-2" type="submit">Save</button>
                                    </div>
                                </div>
                            </form>
                        </div>
