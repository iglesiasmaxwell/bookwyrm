    ...
    <section>
        ...
        <div class="row g-0">
            ...
            <div class="col-sm-12 col-lg-8 col-xl-9 h-screen">
                <nav class="navbar">
                    <div class="container-fluid">
                        <p class="font-body font-semibold text-2xl mx-3 mt-16 text-slate-700 hover:text-slate-700 navbar-brand d-block d-lg-none">Net Library</p>
                    </div>
                    <div class="absolute fixed top-5 right-7 d-none d-xl-block">
                      <div>
                          <a href="{{ url('/home/add/library') }}" class="p-[15px] border-2 border-slate-500 fa fa-plus fa-lg text-slate-500 hover:text-slate-800 hover:bg-slate-500 rounded-lg"></a>
                      </div>
                    </div>
                </nav>
                <div class="xl:mt-20 pb-20 xl:pb-4">
                @forelse ($library_info as $index => $item)
                <div class="bg-slate-300/50 mx-7 my-3 font-body rounded-md shadow-md">
                    <div class="py-3">
                      <div class="d-flex w-100 justify-content-between text-slate-700 px-3">
                        <h5 class="mb-2.5 text-lg font-medium capitalize">{{ $item->library_name }}</h5>
                        <a href="{{ $item->library_url }}" class="uppercase font-bold text-xs hover:text-slate-600 focus:text-slate-600 hover:underline h-fit">{{ preg_replace('#^https://|www.#', '', $item->library_url) }}</a>
                      </div>
                    </div>
                    <div class="d-flex">
                      <a href="{{ route('edit.library', $item->id) }}" class="bg-slate-300/75 hover:bg-slate-300/25 focus:bg-slate-300/25 py-1.5 px-2 border-r border-t border-slate-400/25 w-1/2 rounded-bl-md">
                        <h5 class="font-medium"><i class="fa fa-pencil fa-fw text-slate-700 lg:hover:text-slate-700/50 float-start"></i></h5>
                      </a>
                      <form action="{{ route('delete.library', $item->id) }}" method="POST" class="bg-slate-300/75 hover:bg-slate-300/25 focus:bg-slate-300/25 pt-1.5 px-2 border-t border-slate-400/25 w-1/2 rounded-br-md cursor-pointer">
                        @csrf
                        @method('delete')
                  ...
                  @empty
                    <p class="flex justify-content-center align-self my-44 lg:my-60 font-body text-xl">There's no book on reading</p>
                  @endforelse
                </div>
